...
export default function Comunicazioni() {
  ...
  const add = async () => {
    ...
    await supabase.from("communications").insert({ organization_id: ctx.orgId, title: draft.title, body: draft.body || null, created_by: ctx.userId });
    setDraft({ title: "", body: "" }); setShowNew(false); load();
  };
