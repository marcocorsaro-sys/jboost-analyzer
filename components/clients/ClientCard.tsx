...
export default function ClientCard({
  id, name, domain, industry, status, analyses_count, latest_score, latest_analysis_at,
}: ClientCardProps) {
  ...
  return (
    <Link href={`/clients/${id}`}>
      ...
      >
        {/* Status indicator */}
        {status === 'archived' && (
          <div className="absolute top-3 right-3 text-[10px] uppercase tracking-wider px-2 py-0.5 rounded-full bg-gray-700/50 text-gray-400">
            Archived
          </div>
        )}
        ...
        <div className="p-5">
          ...
          {/* Industry tag */}
          {industry && (
            <span className="inline-block text-[11px] px-2 py-0.5 rounded-full bg-white/5 text-gray-400 mb-3">
              {industry}
            </span>
          )}
