...
// Global timeout guard: 140s (Supabase limit is 150s)
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const SEMRUSH_API_KEY = dbKeys['SEMRUSH_API_KEY'] || Deno.env.get('SEMRUSH_API_KEY') || '';
    const AHREFS_API_KEY = dbKeys['AHREFS_API_KEY'] || Deno.env.get('AHREFS_API_KEY') || '';
    const GOOGLE_PSI_API_KEY = dbKeys['GOOGLE_PSI_API_KEY'] || Deno.env.get('GOOGLE_PSI_API_KEY') || '';
    ...
    if (hasAnyApiKey) {
      const apiResults = await Promise.allSettled([
        ...
        fetchCompanyContext(domain, competitors, targetTopic, dbKeys, dbKeys['PPLX_API_KEY'] || Deno.env.get('PPLX_API_KEY') || '', dbKeys['ANTHROPIC_API_KEY'] || Deno.env.get('ANTHROPIC_API_KEY') || ''),
      ]);
      for (let i = 0; i < apiResults.length; i++) {
        const result = apiResults[i];
        const name = apiNames[i];
        const data = result.status === 'fulfilled' ? result.value : null;
        const isMock = !data || data?._meta?.is_mock === true;
        apiDataMap[name] = data?.data || data || null;
    ...
    if (!hasTime(20000)) {
      ...
      return new Response(JSON.stringify({ error: 'Timeout after API fetching' }), {
    ...
    const OPENAI_API_KEY = dbKeys['OPENAI_API_KEY'] || Deno.env.get('OPENAI_API_KEY') || Deno.env.get('OPEN_AI_API_KEY') || '';
    const ANTHROPIC_API_KEY_VAL = dbKeys['ANTHROPIC_API_KEY'] || Deno.env.get('ANTHROPIC_API_KEY') || '';
    const PPLX_API_KEY = dbKeys['PPLX_API_KEY'] || Deno.env.get('PPLX_API_KEY') || '';
    ...
    console.error('[run-analysis] Fatal error:', error);
    try {
      const supabase = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);
      // Try to extract analysisId from the original request body stored earlier
      // We need a different approach since req.json() was already consumed
      const url = new URL(req.url);
      // Use a broad update for any running analysis that was started recently (within last 3 minutes)
      const threeMinAgo = new Date(Date.now() - 180000).toISOString();
      await supabase.from('analyses')
        .update({ status: 'failed', error_message: `Edge Function error: ${String(error).substring(0, 500)}`, current_phase: 'failed' })
        .eq('status', 'running')
        .lt('started_at', threeMinAgo);
    } catch (_) { console.error('[run-analysis] Failed to mark analysis as failed'); }
...
async function fetchCompanyContext(domain: string, competitors: string[], targetTopic: string, dbKeysMap: Record<string, string>, pplxKey: string, anthropicKey: string) {
  const OPENAI_KEY = dbKeysMap['OPENAI_API_KEY'] || Deno.env.get('OPENAI_API_KEY') || Deno.env.get('OPEN_AI_API_KEY') || '';
